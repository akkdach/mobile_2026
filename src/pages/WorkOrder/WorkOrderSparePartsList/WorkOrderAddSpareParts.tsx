...
const SparePartModal = () => {
  ...
  const SpareParts = () => {
    const renderItem = ({ item, index }: { item: IWorkOrderSparePartStorage; index: number }) => (
      <View
      ...
      >
        ...
        >
          <Image
            source={{ uri: item.sparepartImage }}
            style={{ width: 40, height: 40, borderRadius: 4 }}
            resizeMode="cover"
          />
        </Pressable>

        <Text style={{ flex: 2 }}>{item.matDesc}</Text>
        <Text style={{ flex: 1, textAlign: 'center' }}>{item.znew}</Text>
    ...
    >
      <Dialog.Content>
        <View style={{ alignItems: 'center', justifyContent: 'center' }}>
          {modalImageVisible && (
            <Image
              source={{ uri: activeImageUriPreview }}
              style={{
                width: '100%',
                height: undefined,
                aspectRatio: 1,
