...
const PlanInfo = ({ row }:any) => {
  return (
    <View style={stylesPlan.container}>
      ...
      <Text style={stylesPlan.label}>
        ...
        {row.dateStart ? moment(row.dateStart).format('DD/MM/YYYY') : '—'} เวลา:{' '}
        {row.timeStart ? `${row.timeStart} น.` : '—'}
      </Text>
                        ...
                        <View>
                          <View style={{ flex: 1, alignItems: 'flex-start' }}>
                            ...
                            <Text
                              ...
                              Start Date/Time:{' '}
                              {row.dateFinish
                                ? moment(row.dateFinish).format('DD/MM/YYYY')
                                : ''}{' '}
                              เวลา:{' '}
                              {row.timeFinish ? `${row.timeFinish} น.` : ''}
                            </Text>
                          ...
                          <View style={{ flex: 1, alignItems: 'flex-start' }}>
                            ...
                            <View style={{ marginTop: 0 }}>
                              <CountDown
                                until={Number(
                                  convertDateTimeDeff(row.countDownTimeLimit),
                                )}
  ...
  watch(observe => {
    if (observe.searchText && observe.searchText.length >= 3) {
      if (orderListFilter) {
        setOrderList(
          FullTextSearch(orderListFilter as any[], 'day', observe.searchText) as any,
        );
      }
    } else if (
      ...
      <Modal
        ...
        <View style={{ flexDirection: 'column', paddingBottom: 100 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <View style={{ alignItems: 'center' }}>
              <Button
                style={[styles.btn, { height: 46, borderRadius: 10, width: screenInfo.width > 500 ? 200 : 130, backgroundColor: COLOR.primary }]}
                onPress={() => {
  ...
  const CardWorkOrder = (key: any, args?: IWorkOrder) => {
    return (
      <View
        style={{ flexDirection: 'row', justifyContent: 'space-between' }}
        key={`${args?.day}-${key}`}>
        ...
        <View
          ...
          <View style={{ flexDirection: 'column' }}>
            {args?.orders.map((val: IWorkOrderOrder, idx: number) => {
              return (
                <View style={{ flex: 4 }} key={`${val.orderId}-${idx}`}>
                  <TouchableHighlight
                    onLongPress={() =>
                      Alert.alert(
                        'แจ้งเตือน',
                        'ต้องการเลือกใบงานนี้ใช่หรือไม่ ?',
                        [
                          {
                            text: 'ยกเลิก',
                            style: 'cancel',
                    ...
                    onPress={async () => {
                      ...
                      router.Actions.push(ROUTE.WORKORDERLIST, {
                        ...
                        pmType: val.pmtype,
                        workCenter: userInformation?.wk_ctr,
                        orderTypeDescription: val.orderTypeDescription,
                    ...
                    <View
                      style={[{ marginTop: 10, marginLeft: 4, marginRight: 20 }]}>
                      <View style={{ flexDirection: 'row' }}>
                        <View style={{ flex: 0.1 }}>
                          <View
                            style={{
        ...
        <Controller
          ...
          render={({ field: { onChange, onBlur, value } }) => (
            <TextInput
              ...
              onBlur={onBlur}
              // onBlur={(e: any) => {
              //   const getValue = getValues('searchText');
              //   if (getValue && getValue.length > 0) {
              //     const newResult = (orderListFilter as IWorkOrder[]).map(
              //       val => {
              //         return {
              //           orders: val.orders.map(order => {
              //             return {
              ...
              //     );
              //     setOrderList(
              //       FullTextSearch(newResult as any[], 'day', getValue) as any,
              //     );
              //   } else {
