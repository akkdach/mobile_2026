...
import { getPmChecklistVisiterInspectService } from '../../../../services/work_order_check_list';
...
const InspectorWorkOrderCheckListPMPage = ({ qiSubmit, orderId, errorValidate, workType, webStatus }: Props) => {
  ...
  const getPmChecklist = async () => {
    try {
      let response = await getPmChecklistVisiterInspectService(orderId, workType)
  ...
  const RadioButtonGroup = (
    ...
    return (
      <Card style={styles.marginTop}>
        <Card.Content>
          <Title style={styles.textTitle}>{title}{errorValidate && value === null ? <Text style={{ color: "red", fontSize: screenInfo.width > 500 ? 18 : 14 }}>*</Text> : null}</Title>
          <View style={{ flexDirection: 'row' }}>
            ...
            <View style={{ flex: screenInfo.width < 500 ? 2 : 3 }}>
              <TextInputComponent
                placeholder="หมายเหตุ"
                style={{ height: 42, fontSize: screenInfo.width > 500  ? 16 : 12, borderRadius: 10, paddingLeft: 20 }}
                value={inputValue}
...
export default InspectorWorkOrderCheckListPMPage;
