...
const SparePartTransferPage: React.FC<Props> = props => {
  ...
  const loadAll = async () => {
    try {
      setIsLoading(true);
      const sparePartTransferRequests = await fetchTransferRequestTo();
      const mapSparePartTransferRequest = sparePartTransferRequests.dataResult
        ...
        : [];
      setTransferToList(mapSparePartTransferRequest);
    } catch (error: any) {
  ...
  const loadSparePartTransferRequestTo = async (stge_loc: string) => {
    try {
      setIsLoading(true);
      const sparePartTransferRequestTos = await postSparePartRequest(stge_loc);
      const mapSparePartTransferRequestTo =
        sparePartTransferRequestTos.dataResult
          ? sparePartTransferRequestTos.dataResult.map(val => {
              return {
                label: val,
                value: val,
              };
          ...
          : [];
      setTransferRequestToList(mapSparePartTransferRequestTo);
    } catch (error: any) {
      Alert.alert('แจ้งเตือน', error.message);
    } finally {
      setIsLoading(false);
      ...
      <View style={{flex: 1.5}}>
        <Button
          ...
          onPress={() => {
            ...
            loadSparePartTransferRequestTo(selectTransferTo);
          }}
          style={{
            borderRadius: 50,
            height: 52,
...
export default SparePartTransferPage;
