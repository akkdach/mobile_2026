...
const QualityIndex = (props) => {
  ...
  useEffect(() => {
    (async () => {
      const result = await getQualityIndexMaster("CSD");
      if (result) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await getQualityIndex(orderId, 'CSD');
        console.log('response.defectItem', response.defectItem)
      ...
      Alert.alert('แจ้งเตือน', 'คุณต้องการบันทึกข้อมูล ?', [
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            setIsLoading(true);
            let response = await postQualityIndex(orderId, 'CSD', qiResult.toFixed(2), defectItem, selectedReason ?? '');
            setIsLoading(false);
              ...
              Alert.alert('แจ้งเตือน', 'บันทึกข้อมูลสำเร็จ', [
                {
                  ...
                  onPress: async () => {
                    ...
                    // Actions.replace(ROUTE.QI_CHECK_LIST_NON_CSD, params)
                    navigation.dispatch(StackActions.replace(ROUTE.QI_CHECK_LIST_NON_CSD, params))
                  },
                },
              ]);
            }
          ...
          render={({ field: { onChange, onBlur, value } }) => (
            <TextInput
              ...
              onBlur={(e: any) => {
                ...
                // let nameArr2 = ['qualityIndex_Taste', 'qualityIndex_BrixRatio"', 'qualityIndex_Temp', 'qualityIndex_OvCarbo', 'qualityIndex_Age'];
                let qiResult = [];
