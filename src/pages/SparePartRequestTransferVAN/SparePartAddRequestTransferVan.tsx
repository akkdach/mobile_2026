...
function SparePartAddRequestTransferVan(props: any) {
  ...
  const addRetrive = () => {
    setComponentStorageByItem((previousValue: any) => {
      if (previousValue.countRetrive >= previousValue.maxQuantity) {
          ...
          <Dialog.Content>
            ...
            <View style={styles.modalContent}>
              ...
              <Text style={{fontSize: 45}}>
                {componentStorageByItem?.countRetrive}
              </Text>
              <Icon
                name="plus-circle"
    ...
    return (
      <DataTable.Row key={`${item.material}-${idx}`}>
        ...
        <DataTable.Cell style={[styles.flex1,{justifyContent: 'center'}]}>
          <Text style={{...styles.dataTable_cell, textAlign: 'center'}}>
            {item.quantity}
          </Text>
        </DataTable.Cell>
        <DataTable.Cell style={
          [styles.flex15,{justifyContent: 'center'}]}>
          <Button
          style={{padding:1}}
  ...
  const generateDataTableRow2 = (item: any, idx: number) => {
    return (
      <Grid  key={`${item.material}-${idx}`}>
        <Grid style={{flex: 1}}>
          <Text
            style={{
              ...styles.dataTable_cell,
  ...
  const SpareParts = () => {
    ...
    return (
      <View style={[styles.sparePartBodySection]}>
        <DataTable style={[styles.modalWidth,{marginBottom:300}]}>
...
export default SparePartAddRequestTransferVan;
