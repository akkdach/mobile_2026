...
import { styleSm, styleLg } from './SparePartRequestTransferWiteVanApproveCss';
...
const SparePartRequestTransferVanWiteApprove: FC<InterfaceProps> = (
        ...
        <Dialog
          ...
          <Dialog.Content>
            ...
            <View style={{ flexDirection: 'row', marginTop: 20 }}>
              <Text style={{ flex: 1 }} >เลขที่ขอ : {SparePartRequestHDDetail?.reS_ID}</Text>
              <Text style={{ flex: 2 }}>วันที่ขอ : {Moment(SparePartRequestHDDetail?.reS_DATE).format('d/M/y')}</Text>
              <Text style={{ flex: 1 }}>VAN : {SparePartRequestHDDetail?.wK_CTR}</Text>
              <Text style={{ flex: 1 }}>SLOG : {SparePartRequestHDDetail?.movE_STLOC}</Text>
              <Text style={{ flex: 1 }}>FROM : {SparePartRequestHDDetail?.froM_VAN}</Text>
            </View>
            <View style={{ alignSelf: 'stretch', maxHeight: 500, height: 500 }}>
              {renderDialogTable()}
            </View>
          </Dialog.Content>
          <Dialog.Actions>
  ...
  const generateDataTableRow = (item: ISparePartRequestHD, index: number) => {
    return (
      <DataTable.Row key={`${item?.orderid}-${index}`}>
        ...
        <DataTable.Cell
          style={[styles.flex1, { justifyContent: 'center' }]}>
          <Text
            style={{
              ...styles.dataTable_cell,
            }}>
        ...
        <DataTable.Cell
          style={[styles.flex05, { justifyContent: 'center' }]}>
          <Text
            style={{
              ...styles.dataTable_cell,
            }}>
        ...
        <DataTable.Cell
          ...
          <Text
            ...
            {item.isApprove == 'N' ? 'รออนุมัติ' : 'อนุมัติ'}
          </Text>
        </DataTable.Cell>
        <DataTable.Cell style={[styles.flex2, { justifyContent: 'center' }]}>
          <TouchableOpacity
            style={{ justifyContent: 'center' }}
            activeOpacity={0.9}
            onPress={() => SparePartDetailDialog(item)}>
            <View
              style={[styles.btnDetailSparePart]}>
              <View>
                <Text
  ...
  const renderTable = () => {
    ...
    return (
      <View style={[styles.modalWidth,{ paddingLeft: 5, paddingRight: 5, height: hp(80), marginTop: 20 }]}>
        <ScrollView>
          <DataTable >
            <DataTable.Header style={{ backgroundColor: COLOR.primary }}>
              ...
              <DataTable.Title style={[styles.flex1, { justifyContent: 'center' }]}>
                <Text style={styles.dataTableTitle}>สถานะ</Text>
              </DataTable.Title>
            ...
            {renderTable()}
            {
              <View style={{ alignItems: 'center' }}>
                <Text style={{
                  ...
                  fontFamily: Fonts.Prompt_Medium,
                  color: COLOR.gray,
                }}>ระบบจะแดสงรายการขอโอนอะไหล่ย้อนหลัง 7 วันเท่านั้น</Text>
              </View>
            }
          </>
        }></BackGroundImage>
...
export default SparePartRequestTransferVanWiteApprove;
