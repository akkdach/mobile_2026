...
const SparePartVanCheckListPage: FC<InterfaceProps> = (
  ...
  const loadAll = async () => {
    try {
      ...
      const mapData = checkSparePartTranferRequests.dataResult?.sparepartList
        ? checkSparePartTranferRequests.dataResult?.sparepartList.map(val => {
            let notShow = true;
    ...
    try {
      ...
      Alert.alert('แจ้งเตือน', 'ต้องการบันทึกหรือไม่?', [
        ...
        },
        {
          ...
          onPress: async () => {
            setIsLoading(true);
            const checkSparePartTranferRequests =
              await fetchSparePartListTONumber(
                props.search.selectReceiveTransferFrom,
                selectTONumbers,
                'X',
              );
            setIsLoading(false);
            if (checkSparePartTranferRequests.isSuccess) {
              Alert.alert('แจ้งเตือน', checkSparePartTranferRequests.message, [
                {
                  text: 'ตกลง',
                  onPress: async () => {
                    Actions.pop();
  ...
  const SparePartsDataTable = () => {
    ...
    return (
      <View
        ...
        <DataTable style={[styles.modalWidth]}>
...
export default SparePartVanCheckListPage;
